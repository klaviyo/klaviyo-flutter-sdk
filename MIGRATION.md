...
### Step 3: Update native platform setup

The official SDK has different native setup requirements. Follow the
platform-specific instructions in the
[README](https://pub.dev/packages/klaviyo_flutter_sdk) for:

- **iOS:** Update your `AppDelegate.swift` to import the new plugin module,
  set the notification delegate, and forward notification events.
- **Android:** Update your `MainActivity.kt` to handle push notification
  intents, and declare `KlaviyoPushService` in your `AndroidManifest.xml`.
...
SDK uses a `KlaviyoLocation` object.
...
// After
await KlaviyoSDK().setLocation(KlaviyoLocation(
...
| `.setAddress1(addr)` | `.setLocation(KlaviyoLocation(address1: addr))` |
| `.setAddress2(addr)` | `.setLocation(KlaviyoLocation(address2: addr))` |
| `.setCity(city)` | `.setLocation(KlaviyoLocation(city: city))` |
| `.setCountry(country)` | `.setLocation(KlaviyoLocation(country: country))` |
| `.setRegion(region)` | `.setLocation(KlaviyoLocation(region: region))` |
| `.setZip(zip)` | `.setLocation(KlaviyoLocation(zip: zip))` |
| `.setLatitude(lat)` | `.setLocation(KlaviyoLocation(latitude: lat))` |
| `.setLongitude(lng)` | `.setLocation(KlaviyoLocation(longitude: lng))` |
| `.setTimezone(tz)` | `.setLocation(KlaviyoLocation(timezone: tz))` |
