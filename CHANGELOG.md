...
- **Android automatic push-open tracking can now be disabled**: The plugin's automatic "Opened Push" reporting to Klaviyo now respects the native `com.klaviyo.push.automatic_push_open_tracking` manifest `meta-data` key — set it to `false` to disable it (useful if you need to withhold push data until a profile is identified). This does not affect the Dart `push_notification_opened` stream, which never reaches Klaviyo's backend. This is **non-breaking** — the default is unchanged. The `MainActivity.kt` manual `Klaviyo.handlePush()` setup step some integrations carried over is no longer necessary (and now bypasses this opt-out if kept) — see the [README](README.md#android-setup).
